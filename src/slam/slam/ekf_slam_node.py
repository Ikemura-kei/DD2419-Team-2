...
class EKF_SLAM_Node(Node):
    def __init__(self) -> None:
        ...
        self.R[:3, :3] = np.diag([0.873**2, 0.72**2, 10.6**2])
        self.Q = np.diag([0.001**2, 0.001**2]) # measurement noise
    ...
    def scan_cb(self, scan:LaserScan):
        ...
        for i, line in enumerate(lines):
            ...
            line_markers.markers.append(self.prep_line_marker(g, b, s[i,0], s[i,1], e[i,0], e[i,1], self.line_id))
            pnt_markers.markers.append(self.prep_point_marker(g, b, z[i,0], z[i,1], self.pnt_id))
    ...
    def associate(self, mu:np.ndarray, z_hat:np.ndarray, sigma:np.ndarray, z:np.ndarray, b:np.ndarray, outlier_threshold=1.8, new_landmark_threshold=2.55):
