...
class Object_classifier(Node):
    ...
    def __init__(self):
        ...
        self.sub_camera_info = self.create_subscription(
            CameraInfo, 
            "/camera_info",     # camera info of rectfied image 
            self.camera_info_callback, 
            10)
        
        self.image_sub = self.create_subscription(
            Image,
            '/image_rect',    # rectifed but monochrome image 
            self.image_callback,
            10)
        ...
        self.bb_pub = self.create_publisher(
            BoundingBoxArray, 
            "/detection/arm_camera_bounding_boxes", 
            10)
        
        self.image_bb_pub = self.create_publisher(
            Image, 
            "/detection/arm_camera_image_with_bounding_boxes", 
            10)
    ...
    def compute_bb(self, stamp, frame_id, depth_image, cv_image, t0):     
        ...
        with torch.no_grad():
            ...
            for bb in bbs[0]:
                ...
                if bb_msg.category_name is not None and depth > 0.15 and depth < 2 and bb["width"]*bb["width"] > 100 and y_bb+bb["height"]/2 < 425:
                    
                    point = Point()
                    center_point_x, center_point_y  = int(x_bb+bb["width"]/2), int(y_bb+bb["height"]/2)
                    point.x = float(center_point_x)
                    point.y = float(center_point_y)
                    point.z = 0.0
                    ...
                    text_coordinates_x =  f'X(m)={round(point.x, 2)}' + f' X(pixel)={round(center_point_x, 2)}'
                    text_coordinates_y =  f'Y(m)={round(point.y, 2)}' + f' Y(pixel)={round(center_point_y, 2)}'
